...
def test_sloshing(answer_store):
    ...
    thermal_model = CIESourceModel("apec", 0.1, 11.0, 10000, 0.3, thermal_broad=False, prng=prng)
    n_photons1, n_cells1 = make_photons("photons1", sphere, redshift, A, exp_time, thermal_model)
    ...
    n_photons2, n_cells2 = make_photons("photons2", sphere, redshift, A, exp_time, thermal_model)
    ...
    with (
        ...
        for k in f["data"]:
            assert_array_equal(f["data"][k][()], np.concatenate([f1["data"][k][()], f2["data"][k][()]]))
