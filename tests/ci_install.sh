...
if [[ ${mode} == "testing" ]]; then
    ...
    curl -O http://yt-project.org/data/enzo_tiny_cosmology.tar.gz
    tar -zxf enzo_tiny_cosmology.tar.gz
    curl -O http://yt-project.org/data/GasSloshingLowRes.tar.gz
    tar -zxf GasSloshingLowRes.tar.gz
    curl -O http://yt-project.org/data/FIRE_M12i_ref11.tar.gz
    tar -zxf FIRE_M12i_ref11.tar.gz
    
    # Download answers
    
    curl -O http://hea-www.cfa.harvard.edu/~jzuhone/${ANSWER_VER}.tar.gz
